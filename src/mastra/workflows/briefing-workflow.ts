...
const runBriefingStep = createStep({
  id: 'run-briefing',
  description: 'Triggers the briefing agent to fetch, score and send the daily AI news',
  ...
  execute: async ({ mastra }) => {
    ...
    const response = await agent.generate([
      { role: 'user', content: `Ejecuta el briefing diario de inteligencia artificial. Hoy es ${today}.` },
    ]);
